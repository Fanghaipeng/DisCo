...
from .unet_3d_condition_mm import UNet3DConditionModel
...
class TemConvNet(Net):
    ...
    def freeze_pretrained_part_in_ddpm(self):
        ...
        if self.args.freeze_unet:
            ...
            elif self.args.unet_unfreeze_type == "motion":
                for param_name, param in self.unet.named_parameters():
                    if "motion" not in param_name:
                        param.requires_grad_(False)
                    else:
                        param.requires_grad_(True)
                        param_unfreeze_num += 1

            elif self.args.unet_unfreeze_type == "tem_motion":
                for param_name, param in self.unet.named_parameters():
                    if "tem_" not in param_name and "motion" not in param_name:
                        param.requires_grad_(False)
                    else:
                        param.requires_grad_(True)
                        param_unfreeze_num += 1
    ...
    #                 shape, generator=generator, device=self.device, dtype=self.dtype
    #             )
    #             latents = latents.repeat(1, 1, nframes, 1, 1)
    #     else:
    #         if latents.shape != shape:
    #             raise ValueError(
    #                 f"Unexpected latents shape, got {latents.shape}, expected {shape}"
