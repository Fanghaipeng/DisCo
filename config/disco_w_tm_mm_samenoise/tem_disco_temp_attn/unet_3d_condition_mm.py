...
class UNet3DConditionModel(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin):
    ...
    def __init__(
        ...
        use_inflated_groupnorm: bool = False,
        #! Additional Motion
    ...
    def from_pretrained_2d(cls, pretrained_model_path, subfolder=None):
        ...
        config["use_motion_module"]:             bool = False
        config["motion_module_resolutions"]:     Tuple[int] = ( 1,2,4,8 )
        config["motion_module_mid_block"]:       bool = False
        config["motion_module_decoder_only"]:    bool = False
        config["motion_module_type"]:            str = "Vanilla"
        config["motion_module_kwargs"]:          dict = {
        ...
        config["unet_use_cross_frame_attention"]:bool = False
        config["unet_use_temporal_attention"]:bool = False
        ...
        motion_file = os.path.join("/data/fanghaipeng/project/DYL/Resources/checkpoints/v3_sd15_mm.ckpt")
        if not os.path.isfile(motion_file):
            raise RuntimeError(f"{motion_file} does not exist")
        motion_dict = torch.load(motion_file, map_location="cpu")
        
        for k, v in motion_dict.items():
            if "motion" in k and "proj_out" not in k:
                state_dict.update({k: v})
