...
export AZFUSE_USE_FUSE=0 NCCL_ASYNC_ERROR_HANDLING=0  CUDA_VISIBLE_DEVICES=3
echo $CUDA_VISIBLE_DEVICES

python -m torch.distributed.launch --nproc_per_node=1 --master_port=36688 --use_env finetune_sdm_yaml.py   \
    --cf config/disco_w_tm/yz_tiktok_S256L16_xformers_tsv_temdisco_temp_attn.py \
    --do_train  \
    --root_dir runtest/ \
    --local_train_batch_size 2 \
    --local_eval_batch_size 2 \
    --log_dir runtest/exp/tiktok_temporal_ft_test \
    --epochs 20 --deepspeed \
