...
export MASTER_PORT=26666
echo $CUDA_VISIBLE_DEVICES

mpirun -np 6 python finetune_sdm_yaml.py   \
    --cf config/disco_w_tm_mm/yz_tiktok_S256L16_xformers_tsv_temdisco_temp_attn.py \
    --do_train  \
    --root_dir runtest/ \
    --local_train_batch_size 2 \
    --local_eval_batch_size 2 \
    --log_dir runtest/exp/s3_temmotion_t_motion_nosame_loadnomm \
    --epochs 20 --deepspeed \
    ...
    --drop_ref 0.05  \
    --guidance_scale 1.5 \
    --pretrained_model /data/fanghaipeng/project/DYL/DisCo/runtest/exp/s3_tem_t_motion_nosame_nomotion/53999.pth/mp_rank_00_model_states.pt \
